...
class AppManager
{
     ...
     * @throws \Exception
     */
    public function renderSharePage($debug = false): void
    {
        $this->getSmartLink()->renderSharePage($debug);
    }

    /**
     * Returns the maximum number of users, the customer can export for the current app. For templates and versions 0 is
     * returned as a value is not defined
     */
    public function getUserLimit(): int
    {
        return (int)$this->primaryEntity->getInfo('user_limit') ?? 0;
    }
    ...
    {
        if ($this->primaryEntity instanceof App) {
            return $this->primaryEntity->getStartDate();
        }
    ...
    {
        if ($this->primaryEntity instanceof App) {
            return $this->primaryEntity->getExpiryDate();
        }
