...
class Css {
	 ...
	 */
	function __construct( $cache_dir, App $app, $lang = "de_DE", $file_id = "style", $root_path = "" ) {
		$this->app       = $app;
		$this->lang      = $lang;
	...
	public function getCompiledCss() {
		if ( ! $this->cache->exists( $this->cache_key ) ) {
			...
			foreach ( $this->config_values as $config_id ) {
				$value = $this->app->getConfig( $config_id, array( "value", "type", "compiler" ) );
	...
	public function setFileId( $file_id ) {
		$this->file_id   = $file_id;
		$this->cache_key = "apps_" . $this->app->getId() . "_" . $this->lang . "_" . $this->file_id . ".css";
	}
	...
	public function getCacheKey( $file_id = false ) {
		...
		return "apps_" . $this->app->getId() . "_" . $this->lang . "_" . $file_id . ".css";
	}
